...
# END:base
  ...
  # START:download_tb
  exec { "download_tb":
    command => "wget -O /tmp/torquebox.zip http://torquebox.org/release/org/torquebox/torquebox-dist/3.1.1/torquebox-dist-3.1.1-bin.zip",
    path => $path,
  ...
  # START:symlink_tb
  file { $tb_home:
      ensure => link,      target => "${tb_home}-${tb_version}",      require => Exec["unpack_tb"]
  }
  # END:symlink_tb
